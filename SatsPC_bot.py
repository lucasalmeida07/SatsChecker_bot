...
def main():
    ...
    def helping(message):
        bot.reply_to(message, '''
                SatsPriceChecker é um bot criado para auxiliar em operações de trade de Bitcoin. Aqui serão listadas as funcionalidades disponíveis nesta versão(0.2):
    ...
    def showprice(message):
        price = getbtcprice()
        if price != None:
            bot.reply_to(message, f'O Bitcoin atualmente está sendo negociado por R${price:.8f}')
        else:
    ...
    def showsats(message):
        price = getsatsprice()
        if price != None:
            bot.reply_to(message, f'1 satoshi está sendo negociado hoje por R${getsatsprice():.8f}')
        else:
    ...
    def sats_convertion(message):
        ...
        if price != None:
            bot.reply_to(message, f'A quantia dada pode ser negociada por R${price:.8f}')
        else:
